...
class UploadedLayerSerializer(serializers.ModelSerializer):
    """Serializer for uploaded spatial layers (GeoJSON, GPX, KML, CSV, Shapefile)."""
    ...
    class Meta:
        model = UploadedLayer
        fields = [
            'id', 'name', 'layer_type', 'geojson', 'geojson_data',
            'color', 'visible', 'feature_count',
            'uploaded_by', 'uploaded_by_username', 'uploaded_by_id',
            'uploaded_at', 'updated_at',
        ]
        read_only_fields = ['uploaded_by', 'feature_count', 'uploaded_at', 'updated_at']

    def get_geojson(self, obj):
        """Return GeoJSON FeatureCollection for frontend map display."""
        return obj.geojson_data
