...
class UploadedLayerViewSet(viewsets.ModelViewSet):
    ...
    def upload_geojson(self, request):
        ...
        # GEOSGeometry accepts GeoJSON string with srid=4326 [^24^]
        geometries = []
        ...
        # Create GeometryCollection for mixed geometry types [^37^]
        try:
...
class StatsViewSet(viewsets.ViewSet):
    ...
    def _compute_timeline(self):
        ...
        plots = Plot.objects.order_by('date')
        date_map = defaultdict(lambda: {'plots': 0, 'samples': 0})
        for p in plots:
            date_map[p.date.isoformat()]['plots'] += 1
            date_map[p.date.isoformat()]['samples'] += p.soil_samples.count()
        sorted_dates = sorted(date_map.keys())
        cum_plots, cum_samples = 0, 0
        result = {'dates': [], 'cumulative_plots': [], 'cumulative_samples': []}
