...
# ═══════════════════════════════════════════════════════════════
#  UPLOADED SPATIAL LAYERS (GeoJSON, GPX, KML, CSV, Shapefile)
# ═══════════════════════════════════════════════════════════════

class UploadedLayer(models.Model):
    """Spatial layer uploaded by users for planned data collection visualization."""
    ...
    # Store original GeoJSON FeatureCollection
    geojson_data = models.JSONField(default=dict)

    # PostGIS geometry field (GeometryCollection for mixed types: points, lines, polygons)
    geometry = models.GeometryCollectionField(srid=4326, null=True, blank=True)

    # Display properties
    color = models.CharField(max_length=7, default='#c4a84a')
    ...
    def save(self, *args, **kwargs):
        # Count features from GeoJSON
        if self.geojson_data and 'features' in self.geojson_data:
            self.feature_count = len(self.geojson_data['features'])

        # Convert GeoJSON features to PostGIS GeometryCollection
        ...
        try:
            if self.geojson_data and 'features' in self.geojson_data:
                ...
                if geometries:
                    self.geometry = GeometryCollection(*geometries, srid=4326)
        except Exception as e:
            print(f"Warning: Failed to create PostGIS geometry for layer {self.name}: {e}")

        super().save(*args, **kwargs)

    @property
    def geojson(self):
        """Return GeoJSON FeatureCollection for frontend display."""
