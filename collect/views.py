...
class PlotViewSet(viewsets.ModelViewSet):
    """Plot CRUD with public read access.
    Anyone can list/retrieve plots. Only authenticated users can create/update/delete.
    """
    ...
    def submit_survey(self, request):
        """Accept survey submission from mobile app and create Plot with all related data."""
        logger.info(f"Survey submission received from user: {request.user.username}")
        ...
        if not serializer.is_valid():
            ...
            return Response({
                'instance_id': request.data.get('instance_id', 'unknown'),
                'status': 'error',
        ...
        try:
            ...
            if gps and gps.get('lat') and gps.get('lng'):
                location = Point(float(gps['lng']), float(gps['lat']), srid=4326)
            else:
                location = None
            ...
            plot = Plot.objects.create(
                plot_id=form_data.get('plot_id', f"SURVEY-{instance_id[:8]}"),
                site_name=form_data.get('site_name', 'Untitled Survey'),
                province=form_data.get('province', 'eastern_cape'),
                date=survey_date,
                enumerator=data.get('enumerator', form_data.get('enumerator', request.user.username)),
                location=location,
                altitude=self._safe_float(form_data.get('altitude_manual')) or None,
                gps_accuracy=self._safe_float(form_data.get('plot_gps_accuracy')) or None,
                gps_point_number=self._safe_int(form_data.get('gps_point_number', 1), 1),
                plot_radius=25,
                ...
                trees_count_est=self._safe_int(form_data.get('trees_count_est')) or None,
                canopy_cover=self._safe_int(form_data.get('canopy_cover_est')) or None,
            ...
            if isinstance(tree_measurements, dict):
            ...
            for idx, t in enumerate(tree_measurements):
                TreeMeasurement.objects.create(
                    ...
                    tree_num=idx + 1,
                    gps_waypoint_number=self._safe_int(t.get('gps_waypoint_number'), idx + 1),
                    species=t.get('tree_species', 'other'),
                    tree_status=t.get('tree_status', 'live'),
                    dbh=self._safe_float(t.get('dbh', 0), 0),
                    height=self._safe_float(t.get('height', 0), 0),
                )

            # Create Transects (with species observations and grass samples)
            transects_data = form_data.get('transects', [])
            if isinstance(transects_data, dict):
                transects_data = [transects_data]
            for idx, tr_data in enumerate(transects_data):
                transect = Transect.objects.create(
                    plot=plot,
                    transect_id=tr_data.get('transect_id', 't1'),
                    transect_num=idx + 1,
                )

                # Create Species Observations for this transect
                species_obs_data = tr_data.get('species_obs', [])
                if isinstance(species_obs_data, dict):
                    species_obs_data = [species_obs_data]
                for s_idx, s in enumerate(species_obs_data):
                    SpeciesObservation.objects.create(
                        transect=transect,
                        observation_num=s_idx + 1,
                        gps_waypoint_number=self._safe_int(s.get('gps_waypoint_number'), s_idx + 1),
                        bio_type=s.get('bio_type', 'tree'),
                        species=s.get('species', 'other'),
                        abundance=self._safe_int(s.get('abundance', 1), 1),
                        threat_status=s.get('threat_status', ''),
                        dbh_bio=self._safe_float(s.get('dbh_bio')) or None,
                        aspect=s.get('aspect', ''),
                        landform=s.get('landform', ''),
                        degradation_level=s.get('degradation_level', ''),
                        erosion=s.get('erosion', ''),
                        water_distance=s.get('water_distance', ''),
                        micro_habitat_notes=s.get('micro_habitat_notes', ''),
                    )

                # Create Grass Biomass Samples for this transect
                grass_samples_data = tr_data.get('grass_samples', [])
                if isinstance(grass_samples_data, dict):
                    grass_samples_data = [grass_samples_data]
                for g_idx, g in enumerate(grass_samples_data):
                    GrassBiomassSample.objects.create(
                        transect=transect,
                        sample_num=g_idx + 1,
                        gps_waypoint_number=self._safe_int(g.get('gps_waypoint_number'), g_idx + 1),
                        grass_species=g.get('grass_species', 'mixed'),
                        grass_weight=self._safe_float(g.get('grass_weight', 0), 0),
                        last_burned=g.get('last_burned', ''),
                        grass_notes=g.get('grass_notes', ''),
                    )
            ...
            for idx, s in enumerate(soil_data):
                SoilSample.objects.create(
                    ...
                    point_num=idx + 1,
                    gps_waypoint_number=self._safe_int(s.get('gps_waypoint_number'), idx + 1),
                    depth_class=s.get('soil_depth_class', 'topsoil'),
                    depth_cm=self._safe_float(s.get('soil_depth_cm', 0), 0),
                    soil_texture=s.get('soil_texture', 'loam'),
                    bulk_density=self._safe_float(s.get('bulk_density')) or None,
                    soil_ph=self._safe_float(s.get('ph')) or None,
                )
            ...
            if form_data.get('module_water') == 'yes':
                ...
                if water_gps and water_gps.get('lat') and water_gps.get('lng'):
                    w_gps = Point(float(water_gps['lng']), float(water_gps['lat']), srid=4326)

                extraction_uses = form_data.get('water_extraction_uses', [])
                if isinstance(extraction_uses, list):
                    extraction_uses = ','.join(str(u) for u in extraction_uses)
                ...
                WaterAssessment.objects.create(
                    ...
                    water_ph=self._safe_float(form_data.get('water_ph')) or None,
                    water_temperature=self._safe_float(form_data.get('water_temperature')) or None,
                    water_turbidity_ntu=self._safe_float(form_data.get('turbidity_ntu')) or None,
                    electrical_conductivity=self._safe_float(form_data.get('electrical_conductivity')) or None,
                    ...
                    wetland_width_m=self._safe_int(form_data.get('wetland_width_m')) or None,
            ...
            if form_data.get('module_socioeconomic') == 'yes':
                ...
                SocioEconomic.objects.create(
                    ...
                    households_nearby=self._safe_int(form_data.get('households_nearby')) or None,
                    population_estimate=self._safe_int(form_data.get('population_estimate')) or None,
            ...
            logger.info(f"Survey {instance_id} saved successfully as plot {plot.plot_id}")
            return Response({
            ...
            logger.error(f"Survey submission failed for {instance_id}: {str(e)}")
...
class TreeViewSet(viewsets.ModelViewSet):
    """Tree measurements - public read, authenticated write."""
...
class SoilSampleViewSet(viewsets.ModelViewSet):
    """Soil samples - public read, authenticated write."""
...
class WaterAssessmentViewSet(viewsets.ModelViewSet):
    """Water assessments - public read, authenticated write."""
...
class SocioEconomicViewSet(viewsets.ModelViewSet):
    """Socio-economic data - public read, authenticated write."""
...
class UploadedLayerViewSet(viewsets.ModelViewSet):
    ...
    Public read access - anyone can view layers.
    ...
    def get_queryset(self):
        """Return all visible layers for public read."""
        user = self.request.user
        if user.is_authenticated:
            return UploadedLayer.objects.filter(uploaded_by=user)
        return UploadedLayer.objects.filter(visible=True)
...
class StatsViewSet(viewsets.ViewSet):
    ...
    def _compute_summary(self):
        ...
        # Build canopy vs pH data for bubble chart
        canopy_ph_data = []
        for p in plots:
            if p.soil_samples.count() > 0:
                avg_ph = sum(float(s.soil_ph or 0) for s in p.soil_samples.all()) / p.soil_samples.count()
                canopy_ph_data.append({
                    'x': p.canopy_cover or 0,
                    'y': round(avg_ph, 2),
                    'r': p.trees.count() * 1.5 + 4,
                    'forest_type': p.forest_type,
                })

        # Compute timeline data for frontend charts
        ...
        return {
            ...
            'canopy_ph_data': canopy_ph_data,
            # Timeline data - keys mapped to frontend expectations
            'timeline_labels': timeline.get('dates', []),
            'timeline_plots': timeline.get('cumulative_plots', []),
            'timeline_samples': timeline.get('cumulative_samples', []),
            'timeline_trees': timeline.get('cumulative_trees', []),
            'timeline_water': timeline.get('cumulative_water', []),
            'timeline_socio': timeline.get('cumulative_socio', []),
            'timeline_species_obs': timeline.get('cumulative_species_obs', []),
            'timeline_grass': timeline.get('cumulative_grass', []),
        }
